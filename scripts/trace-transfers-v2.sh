...
page = int("$PAGE".strip() if "$PAGE".strip() else "0")
key = "$KEY"
...
try:
    d = json.load(open("$OUT/v2-page-${PAGE}.json"))
except:
...
matches = 0
match_file = open("$OUT/v2-transfer-matches.jsonl", "a")

for tx in txns:
    tx_str = json.dumps(tx)

    # Does this transaction involve the CoinAegis key?
    ...
    for ekey, ename in exchange_keys.items():
        if ekey in tx_str:
            ...
            # Try to extract transfer amounts from events
            events = {}
            ...
            for eid, ev in events.items():
                ...
                # Check template for transfer clues
                tmpl = ev.get("template_id") or ev.get("templateId") or ""
...
# Get pagination cursor from last transaction
last = txns[-1]
inner_last = last.get("transaction") or last.get("reassignment") or last
...
PYEOF
  ...
  # Handle shell variable substitution issue — use a different approach
  # Actually the heredoc with 'PYEOF' prevents shell expansion. Let me use env vars.

  # Re-do with proper variable passing
